...
class QASystem:
    ...
    def _setup_database(self):
        ...
        try:
            ...
            """)
            self._cursor.execute("""
            CREATE INDEX IF NOT EXISTS idx_group_configs_identifier 
            ON group_configs (group_identifier);
            """)
    ...
    def add_group_invitation_url(self, group_identifier, url):
        """添加或更新群邀请码链接"""
        try:
            # 先尝试更新
            self._cursor.execute("""
            UPDATE group_configs
            SET invitation_url = ?, updated_at = ?
            WHERE group_identifier = ?
            """, (url, self._get_current_timestamp_str(), group_identifier))

            if self._cursor.rowcount == 0:
                # 没有更新到记录，执行插入
                self._cursor.execute("""
                INSERT INTO group_configs (group_identifier, invitation_url, created_at, updated_at)
                VALUES (?, ?, ?, ?)
                """, (group_identifier, url, self._get_current_timestamp_str(), self._get_current_timestamp_str()))

            self._conn.commit()
            self.logger.info(f"Updated invitation URL for group {group_identifier}")
            return True
        except sqlite3.Error as e:
            self._conn.rollback()
            self.logger.error(f"Error updating group config: {e}")
            return False

    def get_group_invitation_url(self, group_identifier):
        """获取群邀请码链接"""
        try:
            self._cursor.execute("""
            SELECT invitation_url FROM group_configs
            WHERE group_identifier = ?
            """, (group_identifier,))
            result = self._cursor.fetchone()
            return result[0] if result else None
        except sqlite3.Error as e:
            self.logger.error(f"Error getting group config: {e}")
            return None